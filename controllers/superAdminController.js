...
const superAdminFunctions = {
  ...
  async viewAllAdmins(req, res) {
    try {
      const results = await db.query(
        "SELECT id,name,email,is_verified,created_at FROM admins"
      );
      if (results.length === 0) {
        res.status(404).json({
          status: 404,
  ...
  async viewOneAdmin(req, res) {
    ...
    const results = await db.query(
      "SELECT id,name,email,created_at,signup_token,is_verified FROM admins WHERE id = $1",
      [id]
    );
    if (results.length === 0) {
  ...
  async superAdminDash(req, res) {
    try {
      ...
      const firstThreeAdmins = await db.query(
        "SELECT name,email FROM admins ORDER BY created_at ASC LIMIT 3"
      );
